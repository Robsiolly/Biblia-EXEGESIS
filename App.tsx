...
const App: React.FC = () => {
  ...
  const handleRead = async (textToRead?: string) => {
    ...
    if (!content && result) {
      content = `${result.verse}. ${result.context}. ${result.theologicalInsights}`;
    }
    ...
    >
      {activeTab === 'studies' ? (
        <div className="space-y-8 animate-in slide-in-from-right duration-500">
          ...
          {loading && (
            <div className="glass p-20 rounded-[3rem] flex flex-col items-center justify-center space-y-6">
              ...
              <p className="text-white/60 font-serif italic text-xl">Consultando Registros Históricos...</p>
            </div>
          )}
            ...
            <div className="space-y-8 animate-in fade-in slide-in-from-bottom-8 duration-700">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                <div className="lg:col-span-2 space-y-8">
                  {imageUrl ? (
                    <div className="rounded-[2.5rem] overflow-hidden glass h-64 md:h-96 shadow-2xl relative border border-white/5">
                      <img src={imageUrl} alt="Cena Histórica" className="w-full h-full object-cover opacity-90" />
                      <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent"></div>
                      <div className="absolute bottom-6 left-8 text-[10px] text-white/40 uppercase tracking-[0.4em]">Reconstrução Visual Gemini</div>
                    </div>
                  ) : imageError ? (
                    <div className="rounded-[2.5rem] glass h-64 md:h-96 flex flex-col items-center justify-center border border-white/5 text-white/20">
                      <i className="fas fa-image-slash text-4xl mb-4"></i>
                      <p className="text-[10px] uppercase tracking-widest">Visualização indisponível para este tema</p>
                    </div>
                  ) : null}
                  ...
                  <div className="glass p-8 md:p-14 rounded-[3rem] space-y-10 border border-white/5 shadow-2xl">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                      ...
                      <div className="space-y-4">
                        <h3 className="text-xl font-bold text-white/90 serif flex items-center gap-2">
                          <i className="fas fa-scroll text-emerald-500/40"></i> Estudo de Termos
                        </h3>
                          ...
                          {result.originalLanguages.map((lang, idx) => (
                            <div key={idx} className="glass bg-white/5 p-4 rounded-2xl border border-white/5">
                              <div className="flex justify-between mb-1">
                                ...
                                <span className="text-[9px] text-white/20 italic tracking-widest">{lang.transliteration}</span>
                              </div>
                ...
                <aside className="space-y-8">
                  <div className="glass p-8 rounded-[2rem] border border-emerald-500/10">
                    <h3 className="text-[10px] font-bold text-emerald-400/60 mb-4 uppercase tracking-widest flex items-center gap-2">
                      <i className="fas fa-brain"></i> Motor Cognitivo
                    </h3>
                    <p className="text-white/40 text-xs leading-relaxed">Gemini 3 Flash otimizado para erudição linguística e arqueologia bíblica.</p>
                  </div>
                </aside>
              </div>
            </div>
          ...
          {!result && !loading && (
            <div className="glass p-24 rounded-[4rem] text-center space-y-8 border border-white/5">
              <i className="fas fa-feather-pointed text-8xl text-emerald-500/10"></i>
              <div className="space-y-4">
                <h2 className="text-4xl font-serif text-white/70">Inicie seu Estudo</h2>
                <p className="text-white/20 text-xs max-w-lg mx-auto uppercase tracking-[0.4em]">Explore a história bíblica através da inteligência artificial avançada.</p>
              </div>
            </div>
          )}
        </div>
        ...
        <div className="space-y-8 animate-in slide-in-from-left duration-500">
          <h2 className="text-2xl font-bold text-white/80 serif px-4">Estudos Anteriores</h2>
